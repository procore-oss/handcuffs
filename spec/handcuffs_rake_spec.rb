...
RSpec.describe 'handcuffs' do
  ...
  after do
    SchemaMigrations.delete_all if ActiveRecord::Base.connection.table_exists?('schema_migrations')
  ...
  describe 'handcuffs:migrate' do
    ...
    context 'with basic config' do
      ...
      context 'when running phase [post_restart]' do
        it 'raises phase out of order error if pre_restart migrations have not run' do
          expect { subject.invoke(:post_restart) }.to raise_error(Handcuffs::PhasesOutOfOrderError)
        end
    ...
    context 'with no default phase' do
      ...
      it 'raises error on nil phase' do
        expect { subject.invoke(:pre_restart) }.to raise_error(Handcuffs::PhaseUndefinedError)
      end
    end
